...
class Main {
    ...
    dump(data) {
        try {
            var date = new Date();
            var today = hTools.padLeft(date.getMonth() + 1, 2, 0) + '-' + hTools.padLeft(date.getDate() + 1, 2, 0);
            fs.appendFile(today + '.log', 'Receive => ' + '\r\n' + data + '\r\n', function(err) {
                if (err) throw err;
            });
        } catch (error) {
            console.log(error);
    ...
    formatMessage(data) {
        ...
        for (let i = 0; i < Object.keys(formats).length; i++) {
            // console.log(customFormats[i].Label + ' => ' + customFormats[i].ContentType + ' => ' + customFormats[i].MaxLen);
            let slicedData = data.slice(lastPosition, lastPosition + formats[i].MaxLen);
            lastPosition = lastPosition + formats[i].MaxLen;
            // console.log(slicedData);
            fields.push(slicedData);
        }
    ...
    getFieldValue(index) {
        try {
            if (typeof this.Msg[index] === 'undefined') {
                throw 'Error => index is not exist.';
            }
